...
CaveBot.Extensions.Tasker.setup = function()
  CaveBot.registerAction("Tasker", "#FF0090", function(value, retries)
    ...
    elseif marker == 3 then -- reporting task
        CaveBot.Conversation("hi", "entregar", "tasks")
        delay(talkDelay*3)
...
local regex2 = "Loot of an ([a-z A-Z]*):"
local taskCompleteRegex = "completou a task"

onTextMessage(function(mode, text)
    -- Verifica se é uma mensagem de loot
    if text:lower():find("loot of") then
        if #regexMatch(text, regex) == 1 and #regexMatch(text, regex)[1] == 3 then
            monster = regexMatch(text, regex)[1][3]:lower():trim()
        elseif #regexMatch(text, regex2) == 1 and #regexMatch(text, regex2)[1] == 2 then
            monster = regexMatch(text, regex2)[1][2]:lower():trim()
        end
        ...
        if monster == m1 or (monster == m2 and storage.caveBotTasker.count) then
            ...
            print("Contagem atualizada: " .. storage.caveBotTasker.count)
        end
    end

    -- Verifica se é uma mensagem de conclusão da task
    if #regexMatch(text, taskCompleteRegex) == 1 then
        local matches = regexMatch(text, taskCompleteRegex)[1]
        local taskMonster = matches[2]:lower():trim()
        local taskName = matches[3]:lower():trim()

        -- Valida se o monstro e o nome da tarefa correspondem
        if taskMonster == storage.caveBotTasker.monster and taskName == storage.caveBotTasker.taskName then
            storage.caveBotTasker.count = storage.caveBotTasker.max
            print("Task concluida! Contagem ajustada para o valor maximo: " .. storage.caveBotTasker.max)
        end
    end
end)