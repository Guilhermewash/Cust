...
TargetBot.Looting.process = function(targets, dangerLevel)
  ...
  -- RE-ORDENAÇÃO DINÂMICA: Garante que vamos sempre no mais perto ATUALMENTE
  if #TargetBot.Looting.list > 1 then
      table.sort(TargetBot.Looting.list, function(a,b) 
          return distanceFromPlayer(a.pos) < distanceFromPlayer(b.pos)
      end)
  end
  ...
  -- Verifica containers já abertos
  ...
  -- Aumentei tries para 50 (era 30) para insistir mais no corpo
  if loot.tries > 50 or loot.pos.z ~= pos.z or dist > maxRange then
  ...
  -- Se estiver perto (dist <= 2), tenta abrir
  ...
  if not container or not container:isContainer() then
    -- Se chegou lá e não tem container, remove da lista
    table.remove(TargetBot.Looting.list, storage.extras.lootLast and #TargetBot.Looting.list or 1)
    return true
  end

  g_game.open(container)
  -- Pequeno aumento no delay para garantir que o server receba o pacote
  waitTill = now + (storage.extras.lootDelay or 300) 
...
TargetBot.Looting.lootContainer = function(lootContainers, container)
  -- Verifica se o container parece vazio (pode ser lag do servidor)
  if container:getItemsCount() == 0 then
      container.retriesEmpty = (container.retriesEmpty or 0) + 1
      -- Se tentou ler menos de 5 vezes (aprox 1 segundo), espera e tenta de novo
      if container.retriesEmpty < 5 then
          waitTill = now + 200 
          return
      end
  end
  ...
  -- Se não tem mais itens para pegar, tenta abrir a próxima backpack dentro (se houver)
  ...
  -- Terminou o loot, remove da lista e fecha
...
onContainerOpen(function(container, previousContainer)
  -- Se estavamos esperando um container abrir, assumimos que é este.
  -- Isso corrige bugs onde o ID do corpo muda (ex: corpo fresco vira podre)
  if waitingForContainer then
...
onCreatureDisappear(function(creature)
  ...
  -- Aumentei a distância de detecção de 6 para 10 (ajuda em DBO com range alto)
  if pos.z ~= mpos.z or math.max(math.abs(pos.x-mpos.x), math.abs(pos.y-mpos.y)) > 10 then return end
  
  schedule(20, function() 
    if not containers[1] then return end
    if TargetBot.Looting.list[30] then return end -- Aumentei limite da lista para 30
    ...
    -- REMOVIDO: if not findPath(...) -> Isso fazia ele ignorar corpos se tivesse mob em volta
    
    table.insert(TargetBot.Looting.list, {pos=mpos, creature=name, container=container:getId(), added=now, tries=0})

    -- Ordena: O menor (mais perto) fica em primeiro [1]
    table.sort(TargetBot.Looting.list, function(a,b) 
      return distanceFromPlayer(a.pos) < distanceFromPlayer(b.pos)
    end)
